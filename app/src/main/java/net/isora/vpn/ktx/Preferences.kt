...
fun PreferenceDataStore.stringNotBlack(name: String, defaultValue: () -> String = { "" }) = PreferenceProxy(name, defaultValue, { key, default ->
    getString(key, default)?.takeIf { it.isNotBlank() } ?: default
}, { key, value ->
    putString(key, value.takeIf { it.isNotBlank() } ?: defaultValue())
})

fun PreferenceDataStore.boolean(name: String, defaultValue: () -> Boolean = { false }) = PreferenceProxy(name, defaultValue, ::getBoolean, ::putBoolean)

fun PreferenceDataStore.int(name: String, defaultValue: () -> Int = { 0 }) = PreferenceProxy(name, defaultValue, ::getInt, ::putInt)

fun PreferenceDataStore.stringToInt(name: String, defaultValue: () -> Int = { 0 }) = PreferenceProxy(name, defaultValue, { key, default ->
    getString(key, "$default")?.toIntOrNull() ?: default
}, { key, value -> putString(key, "$value") })

fun PreferenceDataStore.stringToIntIfExists(name: String, defaultValue: () -> Int = { 0 }) = PreferenceProxy(name, defaultValue, { key, default ->
    getString(key, "$default")?.toIntOrNull() ?: default
